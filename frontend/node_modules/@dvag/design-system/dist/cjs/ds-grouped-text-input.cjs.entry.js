...
const DsGroupedTextInput = class {
  ...
  blurWhenInactive() {
    setTimeout(() => {
      if (this.forceBlur || !miscUtil.isActiveElement(this.self)) {
        this.active = false;
  ...
  render() {
    ...
    return (index.h(index.Host, { title: this.mask, onClick: (e) => {
        if (this.findInnerInputElement(e) == undefined) {
          this.focusFirstUnfulfilledInput();
        }
      }, onBlur: () => {
        this.blurWhenInactive();
      } }, index.h("ds-input-label", { disabled: this.disabled, label: this.label, kisynced: false }), index.h("div", { class: { "input-readonly": this.readonly } }, index.h(inputContainer.InputContainer, { active: this.active, error: isErrorOutline, required: this.required, requirementFulfilled: this.checkFilledOut(), disabled: this.disabled, title: this.internalValue }, this.renderReadonly(), this.renderGroups(), this.renderClearButton(), index.h(errorPopup.ErrorPopup, { text: this.errormessage, show: this.active })), index.h("ds-required-hint", { required: this.required, fulfilled: this.checkFilledOut() }))));
  }
  renderGroups() {
    return this.groups.map((groupOrDelimiter, index$1) => {
      if (groupOrDelimiter instanceof InputGroup) {
        const inputWidthInCharacters = groupOrDelimiter.width;
        const groupedTextInputController = this.allGroupedTextInputController[index$1];
        return (index.h("input", { id: `input-${index$1}`, ref: (el) => {
        ...
        return groupOrDelimiter.delimiter === " " ? (index.h("ds-text", { type: "ft", screensize: "small" }, "\u00A0")) : (index.h("ds-text", { type: "ft", screensize: "small" }, groupOrDelimiter.delimiter));
      }
    });
  }
  renderReadonly() {
    if (this.readonly) {
      ...
      return (index.h("div", { class: "readonly-text" }, index.h(baseElements.ReadonlyTextElement, { text: this.internalValue, kisynced: false })));
    }
    return null;
  }
  renderClearButton() {
    ...
    return (index.h("ds-function-icon-button", { id: elementIdHandling.ElementIdGenerator.createId(this.self, "clear-button"), unfocusable: true, class: {
      ...
      }, onPointerDown: () => this.clearAllInputs() }, index.h("ds-icon", { icon: "schliessen-input", size: 16 })));
  }
...
exports.ds_grouped_text_input = DsGroupedTextInput;
