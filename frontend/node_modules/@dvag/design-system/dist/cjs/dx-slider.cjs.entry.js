...
const DxSlider = class {
  ...
  componentDidRender() {
    renderUtil.onNextFrame(() => this.updatePosition());
  }
  componentWillLoad() {
  ...
  connectedCallback() {
    this.resizeObserver = new ResizeObserver(() => index.forceUpdate(this));
  ...
  updateValueLabelPosition(lineWidth) {
    ...
    this.valueElement.style.transform = `translateX(${mathUtil.clamp((lineWidth - KNOB_WIDTH) * this.relativeValue -
      labelTextWidth / 2 +
  ...
  updateMinMaxLabelVisibility(knobLeft) {
    ...
    new preciseCssSetter.PreciseCssSetter(this.minLabelElement)
      .set("visible", knobRectLeft > minRect.right)
      .execute();
    new preciseCssSetter.PreciseCssSetter(this.maxLabelElement)
  ...
  clickOnSlider(e) {
    ...
    const relativeValue = mathUtil.clamp((e.clientX - left) / (right - left), 0, 1);
    this.applyNewValue(relativeValue);
  }
  ...
  keyDownOnSlider(e) {
    ...
    const relativeValue = mathUtil.clamp((this.value + delta - this.min) / (this.max - this.min), 0, 1);
    this.applyNewValue(relativeValue);
  ...
  checkAsserts() {
    assert.assert(this.min != null, () => "<dx-slider> requires the property 'min'");
    assert.assert(this.max != null, () => "<dx-slider> requires the property 'max'");
    assert.assert(this.min < this.max, () => "<dx-slider> requires 'min' to be lower then 'max'");
    assert.assert(this.value >= this.min && this.value <= this.max, () => "<dx-slider> requires 'value' to be between 'min' and 'max'");
  }
  get usedValue() {
    return mathUtil.clamp(this.value, this.min, this.max);
  }
  /**
  ...
  renderLabel() {
    ...
    if (((_a = this.label) === null || _a === void 0 ? void 0 : _a.length) > 0) {
      return (index.h("div", { class: "label" }, index.h("dx-text", { type: "Infotext-Standard", color: "headline" }, this.label)));
    }
  }
  renderMinMaxLabels() {
    if (this.showminmaxlabel) {
      return (index.h("div", { class: "min-max-labels" }, index.h("dx-text", { type: "Infotext-Small", color: "tags", ref: (el) => (this.minLabelElement = el) }, formatValue(this.min, this.formattingpattern)), index.h("dx-text", { type: "Infotext-Small", color: "tags", ref: (el) => (this.maxLabelElement = el) }, formatValue(this.max, this.formattingpattern))));
    }
  }
  renderValue() {
    if (this.showvaluelabel) {
      return (index.h("div", { class: "value", ref: (el) => (this.valueElement = el) }, index.h("dx-text", { type: "Paragraph-Big", color: "headline", ref: (el) => (this.labelTextElement = el) }, formatValue(this.usedValue, this.formattingpattern))));
    }
  }
  render() {
    this.checkAsserts();
    ...
    return (index.h(index.Host, { class: {
        ...
        "is-safari": isSafari,
      }, tabIndex: 0, onKeyDown: (e) => this.keyDownOnSlider(e), onKeyUp: (e) => this.keyUpOnSlider(e) }, this.renderValue(), index.h("div", { class: "slider", onClick: (e) => this.clickOnSlider(e) }, index.h("div", { class: "line", ref: (el) => (this.lineElement = el) }, index.h("div", { class: "filled", style: { transform: `scaleX(${this.relativeValue})` } })), this.renderMinMaxLabels(), index.h("div", { class: "knob", ref: (el) => (this.knobElement = el), onClick: (e) => e.stopPropagation(), onPointerDown: (e) => this.dragController.onPointerDown(e), onTouchStart: (e) => this.dragController.onTouchStart(e) })), this.renderLabel(), index.h("div", { class: "keyboard-focus" })));
  }
