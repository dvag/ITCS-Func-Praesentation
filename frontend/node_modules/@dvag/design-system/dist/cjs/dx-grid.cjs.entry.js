...
const DxGrid = class {
  ...
  updateGridPool() {
    assert.assert((this.mq1 || this.mq2 || this.mq3 || this.mq4 || this.mq5) != null, () => '<dx-grid> There has to be at least one breakpoint defined: "mq1", "mq2", "mq3", "mq4" or "mq5".');
    this.gridPool = [];
    const gridBase = PAGE_GRID;
    if (this.mq1) {
      this.gridPool.push(new GridSyntaxParser().parse(this.mq1, gridModels.GridBreakpoint.MQ1));
    }
    if (this.mq2) {
      this.gridPool.push(new GridSyntaxParser().parse(this.mq2, gridModels.GridBreakpoint.MQ2));
    }
    if (this.mq3) {
      this.gridPool.push(new GridSyntaxParser().parse(this.mq3, gridModels.GridBreakpoint.MQ3));
    }
    if (this.mq4) {
      this.gridPool.push(new GridSyntaxParser().parse(this.mq4, gridModels.GridBreakpoint.MQ4));
    }
    if (this.mq5) {
      this.gridPool.push(new GridSyntaxParser().parse(this.mq5, gridModels.GridBreakpoint.MQ5));
    }
  ...
  connectedCallback() {
    ...
    this.breakpointAware = new breakpointAware.BreakpointAware(() => this.updateChosenGrid(), true);
  }
  ...
  componentWillRender() {
    var _a;
    sizes_util.assertIsPossibleSize((_a = this.rowgap) !== null && _a !== void 0 ? _a : "16", this.self);
  ...
  getBreakpointFromBreakpointAware() {
    if (this.breakpointAware.mq1) {
      return gridModels.GridBreakpoint.MQ1;
    }
    else if (this.breakpointAware.mq2) {
      return gridModels.GridBreakpoint.MQ2;
    }
    else if (this.breakpointAware.mq3) {
      return gridModels.GridBreakpoint.MQ3;
    }
    else if (this.breakpointAware.mq4) {
      return gridModels.GridBreakpoint.MQ4;
    }
    else {
      return gridModels.GridBreakpoint.MQ5;
    }
  }
  updateChosenGrid() {
    ...
    if (!this.usedGrid) {
      const currentBreakpointMinWidth = gridModels.getBreakpointMinWidth(this.breakpoint);
      this.usedGrid = this.gridPool.reduce((prev, next) => {
        const nextMinWidth = gridModels.getBreakpointMinWidth(next.breakpoint);
        const prevMinWidth = gridModels.getBreakpointMinWidth(prev.breakpoint);
  ...
  render() {
    ...
    return (index.h(index.Host, null, index.h("style", null, result.css), index.h("slot", null)));
  }
  get self() { return index.getElement(this); }
};
DxGrid.style = dxGridCss;

exports.dx_grid = DxGrid;
