...
const states = require('./states-d5e95c84.js');
...
function obtainChildState(items) {
  return items.map((item) => {
    ...
    const highestPriority = allStates.reduce((a, b) => {
      return states.SITE_NAVIGATION_STATE_ARRAY.indexOf(b) <=
        states.SITE_NAVIGATION_STATE_ARRAY.indexOf(a)
        ? b
        : a;
    });
...
const DxSiteNavigationMenu = class {
  ...
  connectedCallback() {
    ...
    this.headerHeightSubscription = headerService.subscribeToHeaderHeight((height, isVisible) => {
  ...
  registerMenuItemObserver() {
    this.menuItemObserver =
      slotObserver.createMetaComponentsSlotObserver(this.self, ":scope > dx-site-navigation-menu-item.hydrated", async (nodes) => {
        ...
        this.menuItems = await Promise.all(nodes.map(async (node) => {
          return Object.assign(Object.assign({}, (await node.toMenuItemModel())), {
            ...
            node.querySelectorAll(queryUtil.notHidden("dx-site-navigation-menu-item"))).map((child) => child.toMenuItemModel()))
          });
        }));
        // Obtain child states
  ...
  clickOnItem(item) {
    ...
    if (this.autonavigation) {
      ...
      this.smallScreenModalVisible = false;
      async.unawaited(this.waitAndScrollToScopeWithValue(item.value));
    }
  ...
  async waitAndScrollToScopeWithValue(value) {
    ...
    for (let i = 0; i < 8 * 5; i++) {
      ...
      if (element) {
        ...
        async.unawaited(this.scrollToElement(siteNavigation));
      }
      await async.sleep(125); // wait between tries
    }
    this.reEnableScrollSelection();
  }
  ...
  async scrollToElement(element, behavior = undefined) {
    ...
    async.unawaited(headerService.HeaderService.instance().disableScrollInteractionForMs(SCROLL_DURATION));
    window.scrollBy({ top: scrollAmount, behavior });
    await async.sleep(SCROLL_DURATION); // scroll duration
  }
  getParentOf(value) {
  ...
  renderItem(item, level = 1) {
    ...
    return [
      index.h("div", { class: {
          ...
          "level-1": level === 1,
          "level-2": level === 2,
        }, onClick: () => this.clickOnItem(item) }, index.h("div", { class: "indicator" }), index.h("div", { class: "indent" }), index.h("div", { class: { state: true, hidden: !this.usesStates } }, index.h("dx-icon", { icon: STATE_ICONS[item.state], color: STATE_COLORS[item.state], size: 16 })), index.h("div", { class: "label" }, index.h("button", { onClick: (e) => {
          e.stopPropagation();
          this.clickOnItem(item);
        }, id: elementIdHandling.ElementIdGenerator.createId(this.self, "button-" + item.value) }, index.h("dx-text", { type: "it", color: "headline" }, item.label))), index.h("div", { class: {
          arrow: true,
        } }, hasChildren ? (index.h("button", { onClick: hasChildren
          ? (e) => {
            e.stopPropagation();
            this.openCloseItem(item);
          }
          : undefined, id: elementIdHandling.ElementIdGenerator.createId(this.self, "open-close-control-" + item.value) }, index.h("dx-chevron", { directiontop: item.value === this.openedMenuItemValue, size: 16, active: item.value === this.selectedvalue }))) : undefined)),
      index.h("dx-simple-grow-box", { slidecontent: true, open: item.value === this.openedMenuItemValue }, children.map((child) => this.renderItem(child, level + 1))),
    ];
  }
  renderLargeScreenMenu() {
    const hasActionSlotItem = renderUtil.isSlotDefined(this.self, "action");
    // check if we need to hide show the separator after grow box (depends on number of visible items)
    renderUtil.onNextFrame(() => setTimeout(() => this.updateScrollabilityOfScrollContainer(), GROWBOX_ANIMATION_TIME));
    return (index.h("div", { class: "large-screen-frame" }, index.h("div", { class: "scroll-container", ref: (el) => {
        this.scrollContainerElement = el;
        this.updateScrollabilityOfScrollContainer();
      } }, this.menuItems.map((item) => this.renderItem(item))), this.scrollContainerIsScrollable ? (index.h("div", { class: "separator" }, index.h("div", { class: "shadow" }), index.h("div", { class: "border" }))) : null, hasActionSlotItem ? (index.h("div", { class: "action" }, index.h("slot", { name: "action" }))) : null));
  }
  renderSmallScreenHeader(position) {
    ...
    return (index.h("div", { class: {
      ...
      }, onClick: () => (this.smallScreenModalVisible = true) }, position === "static" ? (index.h("div", { class: "trigger", ref: (el) => {
        this.smallScreenTriggerElement = el;
        this.configureSmallScreenIntersectionObserver();
      } })) : null, index.h("div", { class: { state: true, hidden: !this.usesStates } }, index.h("dx-icon", { icon: STATE_ICONS[selectedItem.state], color: STATE_COLORS[selectedItem.state], size: 16 })), index.h("div", { class: "label" }, index.h("dx-text", { type: "ps", color: "headline" }, selectedItem.label)), index.h("div", { class: "open-menu" }, index.h("button", { id: elementIdHandling.ElementIdGenerator.createId(this.self, "open-menu") }, index.h("dx-icon", { icon: "ueberlauf-menue", color: "default", size: 24 })))));
  }
  renderSmallScreenMenu() {
    const hasActionSlotItem = renderUtil.isSlotDefined(this.self, "action");
    return [
      this.renderSmallScreenHeader("static"),
      index.h("div", { class: "floating-wrapper", ref: (el) => {
          this.floatingWrapperElement = el;
          this.configureSmallScreenIntersectionObserver();
        } }, this.renderSmallScreenHeader("floating")),
      index.h("dx-modal", { height: "internal-site-navigation-menu", label: this.titlestring, visible: this.smallScreenModalVisible, onModalClosed: () => (this.smallScreenModalVisible = false) }, index.h("div", { slot: "content" }, this.menuItems.map((item) => this.renderItem(item))), hasActionSlotItem ? (index.h("slot", { name: "action", slot: "primary-actions" })) : null),
    ];
  }
  render() {
    const additionalSpaceForHeaderElement = this.headerState.isVisible
      ? this.headerState.height
      : 0;
    return (index.h(index.Host, { class: { "large-screen": this.isLargeScreen }, style: {
        top: `${HEADER_MARGIN + additionalSpaceForHeaderElement}px`,
      } }, this.isLargeScreen
      ? this.renderLargeScreenMenu()
      : this.renderSmallScreenMenu(), index.h("slot", null)));
  }
...
exports.dx_site_navigation_menu = DxSiteNavigationMenu;
