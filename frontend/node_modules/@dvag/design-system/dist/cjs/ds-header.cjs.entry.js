...
const DsHeader = class {
  ...
  connectedCallback() {
    ...
    this.userConfigObserver =
      slotObserver.createMetaComponentsSlotObserver(this.self, "ds-header-user", (nodes) => {
        assert.assert(nodes.length <= 1, () => "<ds-header> Zuviele <ds-header-user> Elemente gefunden");
        if (nodes.length > 0) {
    ...
    this.customerConfigObserver =
      slotObserver.createMetaComponentsSlotObserver(this.self, "ds-header-customer", (nodes) => {
        assert.assert(nodes.length <= 1, () => "<ds-header> Zuviele <ds-header-customer> Elemente gefunden");
        if (nodes.length > 0) {
          nodes[0]
  ...
  disconnectedCallback() {
    ...
    const headerService$1 = headerService.HeaderService.instance();
    if (headerService$1.header === this.self) {
      headerService$1.header = null;
    }
  }
  windowClicked(e) {
  ...
  render() {
    return (index.h(index.Host, null, index.h("div", { class: "header-bar" }, index.h("div", { class: {
        content: true,
        "page-wrapped-content": !this.fullscreen,
      } }, index.h("div", { class: "logo" }, index.h("ds-logo", { type: this.getLogo() })), index.h("div", { class: "center" }, index.h("slot", { name: "center" }), this.renderCustomer()), index.h("div", { class: "right" }, index.h("slot", { name: "right" }), this.renderHeaderItems()))), index.h("slot", { name: "below" })));
  }
  renderHeaderItems() {
    if (this.renderActionItemsInHeader) {
      return (index.h("div", { class: "action-items-in-header" }, this.renderHeaderMenuActionItems(this.headerActionItems)));
    }
    else {
  ...
  renderCustomer() {
    ...
    const delimiter = ": ";
    return (index.h("ds-text", { type: "it" }, this.customerConfiguration.typeLabel, delimiter, this.customerConfiguration.name));
  }
  renderHeaderMenuItems() {
    var _a;
    if (((_a = this.userConfiguration) === null || _a === void 0 ? void 0 : _a.name) != null) {
      return (index.h("div", { class: "user-avatar", ref: (el) => (this.userAvatarElement = el) }, index.h("button", { id: elementIdHandling.ElementIdGenerator.createIdFromString(this.userConfiguration.id, "menu-control"), class: {
          "user-avatar-button": true,
          open: this.userPopupOpen,
        }, onClick: () => this.toggleUserPopup() }, index.h("div", { class: "filler" }), index.h("ds-user-avatar", { class: "avatar-circle", size: "header", imageurl: this.userConfiguration.imageUrl, initials: miscUtil.generateInitialsForName(this.userConfiguration.name) }), index.h("div", { class: "filler" }, index.h("ds-icon", { class: "arrow", size: 16, icon: "drop-down" }))), index.h("div", { class: {
          popup: true,
          open: this.userPopupOpen,
        } }, index.h("div", { class: "popup-inner" }, this.renderHeaderMenuActionItems(this.headerActionItems), index.h("button", { id: elementIdHandling.ElementIdGenerator.createIdFromString(this.userConfiguration.id, "menu-item-control"), class: {
          "popup-head": true,
        ...
        } }, index.h("ds-text", { type: "it", screensize: "small", color: "anthrazit" }, this.userConfiguration.name)), index.h("div", { class: "user-action-item-container" }, this.renderHeaderMenuActionItems(this.userConfiguration.children)), index.h("div", { class: "popup-action" }, index.h("ds-button", { id: "ds-header-btn-toggle-user-popup", size: "small", type: "secondary", onClick: () => {
          this.userConfiguration.signOut();
          this.toggleUserPopup();
        } }, index.h("ds-text", null, "Abmelden")))))));
    }
  }
  renderHeaderMenuActionItems(actionConfigurations) {
    return (index.h("div", { class: "action-item-container" }, actionConfigurations.map((item) => {
      if (headerActionHelper.instanceOfDsHeaderActionConfiguration(item)) {
        return this.renderHeaderMenuAction(item);
      }
      else if (headerActionHelper.instanceOfDsHeaderActionListConfiguration(item)) {
  ...
  renderHeaderMenuAction(item, depth = 0) {
    var _a;
    return [
      index.h("button", { id: elementIdHandling.ElementIdGenerator.createIdFromString(item.id, "control"), class: {
          "header-action-item": true,
          highlight: item.highlight,
        }, onClick: () => {
          item.oninteraction();
        } }, index.h("ds-text", { type: "it" }, (_a = item.label) !== null && _a !== void 0 ? _a : "")),
  ...
  renderHeaderMenuActionList(item, depth = 0) {
    var _a;
    return [
      index.h("button", { id: elementIdHandling.ElementIdGenerator.createIdFromString(item.id, "control"), class: "header-action-item", onClick: () => {
          item.toggleOpen();
        } }, index.h("div", { class: "text-and-arrow" }, index.h("ds-text", { type: "ft" }, (_a = item.label) !== null && _a !== void 0 ? _a : ""), index.h("ds-icon", { class: "arrow", size: 16, icon: "drop-down" }))),
      this.renderSeparator(depth * 25),
    ];
  }
  renderHeaderActionMenuItemsRecursiv(actionListConfiguration, depth = 0) {
    var _a;
    return (index.h("div", { class: { "action-list": true, open: actionListConfiguration.open } }, [
  ...
  renderSeparator(subtractWidth) {
    return (index.h("div", { class: "separator" }, index.h("div", { class: "line", style: { width: `calc(100% - ${subtractWidth}px)` } })));
  }
  get self() { return index.getElement(this); }
};
DsHeader.style = dsHeaderCss;

exports.ds_header = DsHeader;
