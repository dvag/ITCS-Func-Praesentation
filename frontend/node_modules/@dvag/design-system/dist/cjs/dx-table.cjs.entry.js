...
const DxTable = class {
  constructor(hostRef) {
    ...
    this.selectionStateChanged = index.createEvent(this, "selectionStateChanged", 7);
    this.expansionChanged = index.createEvent(this, "expansionChanged", 7);
    this.internalScrollChanged = index.createEvent(this, "internalScrollChanged", 7);
  ...
  onSideBarOpenChange() {
    ...
    index.readTask(() => async.unawaited(this.updateTableScroll(this.self.offsetWidth)));
  }
  ...
  connectedCallback() {
    ...
    this.mutationObserver = new MutationObserver(() => {
      ...
      this.updateHeaderItems(Array.from(headerItems)).then(() => {
        ...
        const expansionEnabled = table_util.containsExpandableRows(this.self);
  ...
  getStickyColumnsWidth() {
    var _a, _b;
    const expansionSpace = table_util.containsExpandableRows(this.self)
  ...
  getScrollWidth() {
    var _a, _b;
    const collapsableSpace = table_util.containsExpandableRows(this.self)
      ? EXPANSION_COLUMN_WIDTH
      : 0;
    const selectionSpace = this.selectionenabled ? SELECTION_COLUMN_WIDTH : 0;
  ...
  updateRows(rows) {
    this.rowDebounce.run(async () => {
      ...
      if (!miscUtil.sameArrayIgnoreOrder(rows, this.rowElements)) {
  ...
  handleRemoteSelection(currentRows) {
    requestAnimationFrame(() => {
      ...
      if (!miscUtil.sameArrayIgnoreOrder(tmpRowsSelected, this.selectedRowKeys)) {
  ...
  async updateHeaderItems(headerItems) {
    ...
    if (this.sameSettings(newSettings) &&
      miscUtil.sameArrayInOrder(this.headerItemElements, headerItems)) {
      return;
    }
    ...
    if (!hadAnySortableColumnsBeforeChange) {
      ...
      if (((_a = this.initialsort) === null || _a === void 0 ? void 0 : _a.length) > 0) {
        ...
        assert.assert(sortedHeaderColumn != null, () => `No <dx-table-header-item> element with 'columnid' or 'label' "${this.initialsort}" found`);
      }
    ...
    index.readTask(() => {
      ...
      if (table != null) {
        async.unawaited(this.updateTableScroll(table.getBoundingClientRect().width));
      }
      this.updateCellSettings().then();
    });
  }
  ...
  render() {
    return (index.h(index.Host, { role: "table", style: {
        ...
        "--dx-table-expansion-column-width": this.expansionEnabled
          ...
          : "0px",
      } }, this.renderPageSizePicker(), index.h("dx-radio-button-group", { radiogroupid: "table-selection-group", ref: (ref) => {
        this.selectionGroup = ref;
      } }, index.h("div", { class: "table-container" }, this.renderTable())), this.renderPaginator()));
  }
  renderPageSizePicker() {
    if (!this.isUsingPagination()) {
      return;
    }
    return (index.h("dx-number-picker", { id: elementIdHandling.ElementIdGenerator.createId(this.self, "page-size-picker"), align: "right", class: "page-size-picker", value: this.pagesize, values: this.pagesizes, onValueChange: (e) => this.handlePaginationPageSizeChanged(e) }));
  }
  renderPaginator() {
    if (!this.isUsingPagination()) {
      return;
    }
    return (index.h("div", { class: "paginator-container" }, index.h("dx-number-paginator", { id: elementIdHandling.ElementIdGenerator.createId(this.self, "paginator"), class: "paginator", onPageChanged: (e) => this.handlePaginationPageChanged(e), current: this.currentPage, count: Math.ceil(this.rowcount / this.pagesize) })));
  }
  renderScrollableTableContent() {
    return (index.h("div", null, index.h("ds-scroll-container", { scrollbarborderradius: 0, thumbwidth: 8, scrollbaroffset: 4 }, index.h("div", { ref: (ref) => {
        index.writeTask(() => {
  ...
  renderTable() {
    const reserveSpaceForCollapsableRows = table_util.containsExpandableRows(this.self);
    return [
      index.h("div", { class: {
          table: true,
