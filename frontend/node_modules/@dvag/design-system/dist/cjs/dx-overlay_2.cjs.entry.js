...
const DxOverlay = class {
  constructor(hostRef) {
    ...
    this.fixedAlignmentChange = index.createEvent(this, "fixedAlignmentChange", 7);
    this.hidePopup = true;
  ...
  internalUpdatePosition() {
    return new Promise((resolve) => {
      if (!this.contentElement ||
        !this.popupElement ||
        !renderUtil.isSlotDefined(this.self, "overlay")) {
        resolve();
        return;
      }
      if (this.fixate) {
  ...
  computeWidth(position, anchorBox, contentBox) {
    const isContentDefined = miscUtil.getNamedSlottedContent(this.self);
  ...
  render() {
    const isHidden = this.hidePopup || !renderUtil.isSlotDefined(this.self, "overlay");
    return (index.h(index.Host, null, index.h("div", { style: {
        display: this.contentdisplay,
      }, ref: (el) => this.obtainContentRef(el) }, index.h("slot", null)), index.h("div", { class: {
        popup: true,
        disabled: this.disabled,
        fixate: this.fixate,
        "is-hidden": isHidden,
        "not-interactive": !this.popupinteractable,
      }, ref: (el) => this.obtainPopupRef(el) }, index.h("slot", { name: "overlay" }))));
  }
...
function closestAnchor(element) {
  return (miscUtil.findUpperElementByPredicate(element, (elem) => {
...
const DxPopup = class {
  ...
  renderPopup() {
    if (this.show) {
      return (index.h("dx-overlay", { position: "bottom", fixate: true, fixatewidth: this.type === "error" ? undefined : 344, onFixedAlignmentChange: (e) => {
      ...
      index.h("div", null), index.h("div", { slot: "overlay", class: "base invisible", ref: (el) => (this.base = el) }, index.h("div", { class: {
          popup: true,
          right: this.internalArrowPosition === "right" ||
            this.internalDirection === "right",
        } }, index.h("div", { class: "arrow-up" }), index.h("div", { class: "bubble", ref: (r) => (this.bubble = r) }, index.h("div", { class: {
          content: true,
          right: this.internalArrowPosition === "right" ||
            this.internalDirection === "right",
        } }, index.h("slot", null)))))));
...
exports.dx_overlay = DxOverlay;
exports.dx_popup = DxPopup;
