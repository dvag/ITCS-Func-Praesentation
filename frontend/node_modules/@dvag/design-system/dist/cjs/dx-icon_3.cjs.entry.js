...
const DxIcon = class {
  ...
  componentDidRender() {
    ...
    assert.assert(possibleSizes.includes(size), () => {
  ...
  renderExternalIcon(size) {
    return (index.h("object", { data: this.icon, type: "image/svg+xml", width: this.size, height: this.size }, index.h("dx-icon-placeholder", { size: size })));
  }
  renderIcon(size) {
    ...
    return index.h(IconTag, null);
  }
  render() {
    assert.assert(this.icon != null, () => "The property 'icon' has to be defined.");
    return (index.h(index.Host, { style: generateStyleMap$1(this.size, this.color) }, this.renderIcon(this.size)));
  }
...
function generateStyleMap$1(size, color) {
  ...
  if (color) {
    map["--dx-icon-color"] = new colorPalette.ColorPalette(true).getColorCode(color);
  }
  return map;
}
...
const DxText = class {
  ...
  disconnectedCallback() {
    this.self.classList.remove(constants.ADD_BULLET_TO_TEXT_CSS_CLASS_NAME); // placed by parent dx-text-list
  }
  computeTextStyleBasedOnTypeProperty() {
    ...
    assert.assert(matchedStyle != null, () => `<dx-text> type has an invalid value: ${this.type}.\n\nPossible values are: ${TEXT_STYLE_MAPPING.map((s) => `\n- ${s.names
      .map((n) => `"${n}"` +
      " (deprecated)".repeat(s.deprecated.includes(n) ? 1 : 0))
      .join(", ")}`)
      .join("")
      .toLowerCase()}\n\n Wird kein type angegeben, wird der Text-Stil automatisch ermittelt.`);
    assert.assert(!((_b = matchedStyle === null || matchedStyle === void 0 ? void 0 : matchedStyle.deprecated) === null || _b === void 0 ? void 0 : _b.includes(this.type)), () => `Found deprecated type ${this.type}! Please use ${matchedStyle.names
  ...
  render() {
    ...
    return (index.h(index.Host, { class: Object.assign({}, styleClasses), style: generateStyleMap(this.color) }, index.h("slot", null)));
  }
  get self() { return index.getElement(this); }
  static get watchers() { return {
    "type": ["typeChanged"]
...
function generateStyleMap(color) {
  if ((color === null || color === void 0 ? void 0 : color.length) > 0) {
    const hexColor = new colorPalette.ColorPalette(true).getColorCode(color);
...
exports.dx_icon = DxIcon;
exports.dx_icon_placeholder = DxIconPlaceholder;
exports.dx_text = DxText;
