...
const DxTag = class {
  ...
  connectedCallback() {
    if (this.type === "static" &&
      ...
      this.color == null &&
      this.textcolor == null) {
      assert.assertPropertyNotEmpty("dx-tag", "theme", this.theme, "Please specify a theme or use the textcolor and color attribute of this element");
    }
  ...
  renderCloseButton() {
    ...
    return (index.h("button", { onClick: (e) => {
        this.removeButtonClicked(e);
      }, class: "close-button" }, index.h("dx-icon", { icon: "schliessen", size: 16, color: "default" }), index.h("div", { class: "extended-click-area" })));
  }
  renderLabel() {
    const textType = this.theme === "gold" || this.selected ? "it-bold" : "it";
    const dataLabelContent = this.selectable ? this.label : null;
    return (index.h("dx-text", { class: { label: this.selectable }, "data-label": dataLabelContent, type: textType }, this.label));
  }
  renderInteractive() {
    return (index.h("button", { onClick: () => this.toggleSelected(), id: elementIdHandling.ElementIdGenerator.createId(this.self, "control"), class: {
        interactive: true,
  ...
  renderStatic() {
    const colorPalette$1 = new colorPalette.ColorPalette(true);
    const backgroundHexColor = this.color
      ? colorPalette$1.getColorCode(this.color)
      : null;
    const textHexColor = this.textcolor
      ? colorPalette$1.getColorCode(this.textcolor)
      : null;
