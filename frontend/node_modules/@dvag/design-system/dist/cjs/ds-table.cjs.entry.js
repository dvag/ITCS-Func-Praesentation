...
const DsTable = class {
  constructor(hostRef) {
    ...
    this.tableStateChanged = index.createEvent(this, "tableStateChanged", 7);
    this.rowsSelected = index.createEvent(this, "rowsSelected", 7);
    this.currentSortSettings = { column: null, reverse: false };
    this.sortDebounce = new debounce.Debounce(50);
    this.rowDebounce = new debounce.Debounce(200);
  ...
  observeRows() {
    this.rowObserver = new slotObserver.SlotObserver(this.self, "ds-table-row", () => {
      this.rowDebounce.run(async () => {
        ...
        if (!miscUtil.sameArrayIgnoreOrder(currentRows, this.rowElements)) {
  ...
  observeHeaderItems() {
    this.headerItemObserver = new slotObserver.SlotObserver(this.self, "ds-table-header-item", async (nodes) => {
      var _a, _b;
      const newSettings = await Promise.all(nodes.map((node) => {
        return node.toColumnSettings();
      }));
      if (!this.differentSettings(newSettings) &&
        miscUtil.sameArrayInOrder(this.headerItemElements, nodes)) {
        return;
      }
      ...
      if (!hadAnySortableColumnsBeforeChange) {
        ...
        if (((_a = this.initialsort) === null || _a === void 0 ? void 0 : _a.length) > 0) {
          ...
          assert.assert(sortedHeaderColumn != null, () => `Kein <ds-header-item-element> mit der 'columnid' oder dem 'label' "${this.initialsort}" gefunden`);
        }
  ...
  renderCheckboxSelection(rows) {
    return (index.h("div", null, rows.map((row, index$1) => {
      return (index.h("div", { class: {
          ...
          height: row.offsetHeight < DEFAULT_ROW_HEIGHT
            ...
            : row.offsetHeight + "px",
        } }, row.rowid ? (index.h("ds-checkbox", { id: elementIdHandling.ElementIdGenerator.createId(this.self, `selection-button-${index$1}`), checked: this.selectedRowKeys.includes(row.rowid), onCheckedChange: (e) => this.handleCheckboxSelection(row.rowid, e.detail) })) : null));
    })));
  }
  renderRadioSelection(rows) {
    return (index.h("ds-radio-button-group", { id: elementIdHandling.ElementIdGenerator.createId(this.self, "selection-group"), ref: (ref) => (this.selectionGroup = ref), onValueChange: (e) => this.handleRadioSelection(e.detail) }, rows.map((row, index$1) => {
      return (index.h("div", { class: {
          row: true,
          odd: index$1 % 2 === 0,
        }, style: {
          height: row.offsetHeight < DEFAULT_ROW_HEIGHT
            ? `${DEFAULT_ROW_HEIGHT}px`
            : row.offsetHeight + "px",
        } }, row.rowid ? (index.h("ds-radio-button", { id: elementIdHandling.ElementIdGenerator.createId(this.self, `selection-button-${index$1}`), value: row.rowid })) : null));
    })));
  }
  renderSidebarFooter() {
  ...
  renderSidebar() {
    ...
    return (index.h("div", { class: {
      ...
      } }, index.h("div", { class: "sidebar" }, index.h("div", { class: "header", style: {
  ...
  renderScrollableTableContent() {
    return (index.h("div", null, this.renderScrollHint(), index.h("ds-scroll-container", { scrollbarmargin: 8 }, index.h("div", { ref: (ref) => {
  ...
  renderScrollHint() {
    if (this.scrollHintNecessary) {
      return (index.h("div", { class: {
        ...
        }, onTransitionEnd: () => this.scrollHintFadedOutHandler() }, index.h("ds-icon", { icon: "listenpfeil", color: "weiss", size: 32 })));
    }
  ...
  renderTable() {
    return [
      this.renderSidebar(),
      index.h("div", { class: "table", style: {
          "--ds-row-border-left-radius": this.selectionenabled ? "0px" : "6px",
        }, ref: (ref) => {
          var _a;
          (_a = this.tableResizeObserver) === null || _a === void 0 ? void 0 : _a.observe(ref);
        } }, this.internalScrollable ? (this.renderScrollableTableContent()) : (index.h("slot", null)), index.h("div", { class: {
          "left-gradient": true,
  ...
  renderDefault() {
    return index.h(index.Host, { role: "table" }, this.renderTable());
  }
  renderPaginatedCard(shadow) {
    return (index.h(index.Host, { role: "table", class: {
        paginated: true,
        "paginated-shadow": shadow,
      } }, index.h("ds-card", null, index.h("ds-card-header", null, index.h("div", null, index.h("slot", { name: "title" })), index.h("ds-table-page-size-picker", { value: this.pagesize, pagesizes: this.pagesizes, onPageSizeChanged: (e) => this.handlePaginationPageSizeChanged(e) })), index.h("ds-card-content", null, index.h("div", { class: "card-content-bottom-margin" }, this.renderTable())), index.h("ds-hr", null), index.h("div", { class: "footer" }, index.h("ds-paginator", { id: elementIdHandling.ElementIdGenerator.createId(this.self, "paginator"), onPageChanged: (e) => this.handlePaginationPageChanged(e), range: 1, current: this.currentPage, count: Math.ceil(this.rowcount / this.pagesize) })))));
  }
