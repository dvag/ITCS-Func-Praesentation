...
const DxBreadcrumb = class {
  ...
  connectedCallback() {
    this.itemObserver = new slotObserver.SlotObserver(this.self, "dx-breadcrumb-item", async (nodes) => {
  ...
  renderBreadcrumbItem(item, textEllipsis = false, lastItem = false) {
    if (!lastItem) {
      return (index.h(index.Fragment, null, index.h("div", { class: { "breadcrumb-item": true, "with-label": true } }, index.h("button", { class: "breadcrumb-item-button", onClick: () => this.navigate.emit(item.value) }, this.renderBreadcrumbItemText(textEllipsis, item))), this.renderChevron()));
    }
    else {
      return (index.h("div", { class: { "breadcrumb-item": true, ellipsis: textEllipsis } }, this.renderBreadcrumbItemText(textEllipsis, item)));
    }
  }
  renderBreadcrumbItemText(textEllipsis, item) {
    return (index.h("dx-text", { type: "Infotext-Small", class: { "ellipsis-text": textEllipsis } }, item.label));
  }
  renderHomePart() {
    return (index.h(index.Fragment, null, index.h("div", { class: { "breadcrumb-item": true, "with-home-icon": true } }, index.h("button", { class: "breadcrumb-item-button", onClick: () => this.navigateHome.emit() }, index.h("dx-icon", { class: "home-icon", icon: "home", size: 16 }))), this.renderChevron()));
  }
  renderDots() {
    var _a;
    ...
    return (index.h(index.Fragment, null, index.h("div", { class: { "breadcrumb-item": true, "with-dots": true } }, index.h("button", { class: "breadcrumb-item-button", onClick: () => {
        if (previousItem >= 0) {
      ...
      } }, index.h("dx-text", { type: "Infotext-Small", key: "ellipse" }, "..."))), this.renderChevron()));
  }
  renderChevron() {
    return index.h("dx-icon", { class: "chevron-icon", icon: "chevron-rechts", size: 16 });
  }
  render() {
    return (index.h(index.Host, null, index.h("div", { class: "item-measurement-box", ref: (ref) => {
        this.setMeasurementBox(ref);
      } }, index.h("div", { class: "breadcrumb-parts-container" }, this.renderHomePart(), this.renderBreadcrumbItems(this.items))), index.h("div", { ref: (ref) => (this.breadcrumbContainer = ref), class: "width-measurement-box" }, index.h("div", { class: "breadcrumb-parts-container" }, this.renderBreadcrumb()))));
  }
...
exports.dx_breadcrumb = DxBreadcrumb;
