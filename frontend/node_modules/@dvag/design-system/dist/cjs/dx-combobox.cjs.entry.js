...
const DxCombobox = class {
  constructor(hostRef) {
    ...
    this.selectSearchOption = index.createEvent(this, "selectSearchOption", 7);
  ...
  onItemSelectionChanged(e) {
    ...
    if (e.detail.reason === "implicit") {
      ...
      if (miscUtil.isActiveElement(this.inputElement)) {
  ...
  renderInputIcon() {
    var _a, _b;
    return (index.h("dx-horizontal-grow-box", { open: !this.inputFocused && ((_b = (_a = this.selectedOption) === null || _a === void 0 ? void 0 : _a.icon) === null || _b === void 0 ? void 0 : _b.length) > 0, forcewidth: 40, animationmode: "open-from-left" }, index.h("dx-icon", { class: "input-icon", size: 24, icon: this.internalIcon, color: "headline" })));
  }
  renderInput() {
    var _a;
    return (index.h("label", { class: {
      ...
      } }, index.h(inputContainer.LabeledInputContainer, { containerProperties: {
        active: this.inputFocused,
        size: this.size,
        title: this.value,
        error: error.hasError(this.self) || ((_a = this.internalErrorMessage) === null || _a === void 0 ? void 0 : _a.length) > 0,
      }, labelProperties: {
        disabled: this.disabled,
        label: this.label,
        required: this.required,
        kisynced: this.kisynced,
      } }, index.h(baseElements.BaselineElement, null), this.renderInputIcon(), index.h("input", { enterKeyHint: "next", id: elementIdHandling.ElementIdGenerator.createId(this.self, "control"), ref: (el) => (this.inputElement = el), onInput: () => this.onInputInput(), onKeyDown: (event) => this.onInputKeyDown(event), onFocus: () => this.onFocused(), onBlur: () => this.onBlurred(), type: "text", placeholder: this.placeholder, value: this.internalValue, maxLength: this.limitlength, disabled: this.disabled }), this.renderIconAndClearButton()), this.renderAutocompleteValues()));
  }
  renderDefaultContainer(content) {
    return (index.h("div", { class: "default-list-container", key: "default-container" }, content));
  }
  renderListContent() {
    var _a;
    if (this.waitForMoreInput) {
      return this.renderDefaultContainer(index.h("dx-text", null, this.presearchconditiontext ||
        `Bitte geben Sie einen Suchbegriff ein (min. ${this.presearchlettercount} Zeichen).`));
    }
    if (this.isLoading) {
      return this.renderDefaultContainer(index.h("div", { class: "loading-container" }, index.h("dx-spinner", { class: "loading-spinner", size: "medium", theme: "light" }), index.h("dx-text", { color: "headline" }, this.loadingtext)));
    }
    if (((_a = this.searchResult) === null || _a === void 0 ? void 0 : _a.length) > 0) {
      ...
      return this.renderDefaultContainer(index.h("dx-text", { color: "headline" }, this.emptysearchtext));
    }
  }
  renderCategory(category) {
    var _a;
    return [
      category.label ? (index.h("div", { class: "category" }, index.h("dx-text", { type: "it" }, category.label))) : null,
    ].concat((_a = category.options) === null || _a === void 0 ? void 0 : _a.map((option) => {
      const value = getOptionValue(option);
      return (index.h("dx-list-item", { id: elementIdHandling.ElementIdGenerator.createId(this.self, value.toLowerCase().replace(" ", "-")), value: value, prelabel: option.preLabel, label: option.label, sublabel: option.subLabel, icon: option.icon, type: "selectable", selected: getOptionValue(this.selectedOption) === value }));
    }));
  }
  renderAutocompleteValues() {
    var _a;
    return (index.h("dx-list-flyout", { maxheight: this.maxlistheight, defaulthighlightfirst: true, preventSpaceSelection: true, selectonhighlight: false, preventArrowSelection: true, focusable: false, open: this.listOpen, position: "bottom", direction: "right", itemsvisible: 5.5, size: "m", selectionmode: "single", onClick: (event) => this.onListFlyoutClicked(event), class: {
        "respect-label-height": ((_a = this.label) === null || _a === void 0 ? void 0 : _a.length) > 0,
      } }, index.h("div", { slot: "base" }), this.renderListContent()));
  }
  renderOpenListButton() {
    return (index.h("button", { tabIndex: -1, id: elementIdHandling.ElementIdGenerator.createId(this.self, "open-list-button"), class: {
        ...
        this.focusControl();
      } }, index.h("dx-chevron", { directiontop: this.listOpen, active: this.inputFocused })));
  }
  renderIconAndClearButton() {
    var _a;
    const isClearButtonVisible = ((_a = this.internalValue) === null || _a === void 0 ? void 0 : _a.length) > 0;
    return (index.h(index.Fragment, null, index.h("dx-element-swapper", { active: isClearButtonVisible ? "second" : "first" }, index.h("dx-icon", { slot: "first", size: 24, icon: "lupe", color: "default", class: "display-icon" }), index.h("dx-element-swapper", { slot: "second", active: this.isLoading && !this.inputFocused ? "first" : "second" }, index.h("dx-spinner", { slot: "first", size: "medium", theme: "light" }), index.h("dx-clear-button", { tabIndex: -1, slot: "second", visible: true, size: 24, onClearInput: () => this.clearInput(), id: elementIdHandling.ElementIdGenerator.createId(this.self, "clear-button") }))), index.h("dx-spacer", { mq1: "8h" }), index.h("div", { class: "separator" }), index.h("dx-spacer", { mq1: "8h" }), this.renderOpenListButton()));
  }
  render() {
    if (this.readonly) {
      return index.h(baseElements.ReadonlyTextElement, { text: this.value, label: this.label });
    }
    return (index.h(index.Host, { class: { disabled: this.disabled }, onBlur: () => this.onBlurred() }, this.renderInput(), index.h(error.ErrorMessage, { self: this.self, message: this.errormessage || this.internalErrorMessage, custom: {
        onFocus: () => {
          this.preventReFocus = true;
        },
        onBlur: () => {
...
exports.dx_combobox = DxCombobox;
