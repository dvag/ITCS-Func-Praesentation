...
const date = require('./date-2b3d0bd5.js');
...
class InputController {
  ...
  setValue(value) {
    if (this.disabled) {
      value = date_utils.DateUtils.withMultipleLeadingZeros(this.minValue, 4);
    }
  ...
  format() {
    ...
    const newValue = mathUtil.clamp(currentValue, this.minValue, this.maxValue);
...
const DsDateInput = class {
  constructor(hostRef) {
    ...
    this.errorFound = index.createEvent(this, "errorFound", 7);
    this.segmentBlur = index.createEvent(this, "segmentBlur", 7);
    this.dayEmpty = true;
  ...
  componentWillRender() {
    this.value = date_utils.DateUtils.dateToString(this.adjustDateDependantOnCurrentMode(date_utils.DateUtils.newDateFromString(this.value)));
  }
  initalizeInputController() {
    this.yearInputController = new InputController(0, 9999, !this.isYearEnabled(), date_utils.formatToYear, 4);
  ...
  adjustDateDependantOnCurrentMode(date) {
    if (date_utils.DateUtils.isValidDate(date) && !this.isYearEnabled()) {
  ...
  checkValueValidity() {
    let errorType = date.DateErrorType.NO_ERROR;
    if (this.value == null) {
      if (this.isInputValid()) {
        errorType = date.DateErrorType.INVALID_DATE;
      }
    }
    else if (this.min &&
      date_utils.DateUtils.isBeforeDate(new Date(this.value), new Date(this.min))) {
      errorType = date.DateErrorType.UNDER_MIN;
    }
    else if (this.max &&
      date_utils.DateUtils.isAfterDate(new Date(this.value), new Date(this.max))) {
      errorType = date.DateErrorType.OVER_MAX;
    }
  ...
  getErrorMessage(errorType) {
    const errorToMessageMap = {
      [date.DateErrorType.INVALID_DATE]: () => {
      ...
      [date.DateErrorType.UNDER_MIN]: () => `Das eingegebene Datum muss nach dem ${this.getFormattedDisplayDate(this.min)} liegen.`,
      [date.DateErrorType.OVER_MAX]: () => `Das eingegebene Datum muss vor dem ${this.getFormattedDisplayDate(this.max)} liegen.`,
      [date.DateErrorType.NO_ERROR]: () => "",
    };
    return errorToMessageMap[errorType]();
  }
  getFormattedDisplayDate(date) {
    const formattedDate = date_utils.DateUtils.getFormattedDisplayDate(date);
  ...
  formatToDateString(value) {
    ...
    if (isNaN(date.getTime())) {
      ...
      return date_utils.DateUtils.dateToString(date);
    }
  }
  onFocusInput() {
  ...
  onBlur() {
    ...
    renderUtil.onNextFrame(() => {
      if (!miscUtil.isActiveElement(this.self)) {
        this.currentlyEditing = false;
        this.currentFocusedIndex = null;
        this.inputFocused = false;
  ...
  get day() {
    ...
    return date_utils.DateUtils.withMultipleLeadingZeros(day, 2);
  }
  get month() {
    if (this.value == null) {
      return null;
    ...
    return date_utils.DateUtils.withMultipleLeadingZeros(month, 2);
  }
  get year() {
    if (this.value == null) {
      return null;
    ...
    return date_utils.DateUtils.withMultipleLeadingZeros(year, 4);
  }
  ...
  selectDate(date) {
    var _a;
    this.internalValue = date_utils.DateUtils.dateToString(date);
    this.emitValueChange();
  ...
  renderHiddenFallbackInput(inputController) {
    return (index.h("input", { hidden: true, tabIndex: -1, ref: (el) => {
  ...
  renderDayInput() {
    if (this.isDateEnabled()) {
      return [
        index.h("input", { id: elementIdHandling.ElementIdGenerator.createId(this.self, "day-control"), tabIndex: this.currentlyEditing ? 0 : -1, ref: (el) => {
  ...
  renderMonthInput() {
    if (this.isMonthEnabled()) {
      return [
        index.h("input", { id: elementIdHandling.ElementIdGenerator.createId(this.self, "month-control"), tabIndex: this.currentlyEditing ? 0 : -1, ref: (el) => {
  ...
  renderYearInput() {
    if (this.isYearEnabled()) {
      return (index.h("input", { id: elementIdHandling.ElementIdGenerator.createId(this.self, "year-control"), tabIndex: this.currentlyEditing ? 0 : -1, class: {
          year: true,
  ...
  renderInput() {
    ...
    return (index.h("label", { id: elementIdHandling.ElementIdGenerator.createId(this.self, "control") }, index.h("div", null, index.h("dx-overlay", { position: "bottom", offsety: 8, disabled: !this.inputFocused }, index.h(inputContainer.InputContainer, { active: this.inputFocused, error: isErrorOutline, required: this.required, requirementFulfilled: this.isValueValid(), disabled: this.disabled, custom: {
        tabIndex: this.currentlyEditing || this.disabled ? -1 : 0,
        onFocus: () => this.onFocusInput(),
        onBlur: () => this.onBlur(),
        onInput: () => this.checkInputs(),
        onKeyDown: (e) => this.handleKeyInput(e),
      }, title: this.getFormattedDisplayDate(this.value) }, this.renderDayInput(), this.renderMonthInput(), this.renderYearInput(), this.renderIconOrClearButton()), this.renderCalendar(), index.h("ds-required-hint", { required: this.required, fulfilled: this.isValueValid() }))), index.h("ds-input-label", { label: this.label, kisynced: this.kisynced })));
  }
  renderClearButton() {
    return (index.h("ds-function-icon-button", { id: elementIdHandling.ElementIdGenerator.createId(this.self, "clear-button"), unfocusable: true, onBlur: () => {
        this.onBlur();
        this.dayInputController.onBlur();
      }, class: {
        "clear-button": true,
  ...
  renderIconOrClearButton() {
    ...
    return (index.h("div", { class: "right-side" }, isClearButtonVisible ? (this.renderClearButton()) : (index.h("ds-icon", { icon: "kalender", size: 16 }))));
  }
  renderCalendar() {
    var _a, _b;
    ...
    return (index.h("ds-calendar", { slot: "overlay", id: elementIdHandling.ElementIdGenerator.createId(this.self, "calendar"), ref: (ref) => {
  ...
  render() {
    ...
    if (this.readonly) {
      const date = new Date(this.value);
      return (index.h("div", { class: "read-only" }, index.h(baseElements.ReadonlyTextElement, { text: ((_a = this.value) === null || _a === void 0 ? void 0 : _a.length) > 0
...
function formatWithLeadingZero(value) {
  ...
  return date_utils.DateUtils.withLeadingZero(parseInt(value));
}
DsDateInput.style = dsDateInputCss;

exports.ds_date_input = DsDateInput;
