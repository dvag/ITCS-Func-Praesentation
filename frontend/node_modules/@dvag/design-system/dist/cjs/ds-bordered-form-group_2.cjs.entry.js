...
const DsBorderedFormGroup = class {
  ...
  onDirectionChange() {
    renderUtil.onNextFrame(() => {
      this.computeBorders();
      async.unawaited(this.refreshWidth());
    });
  }
  onRequirementFulfilled() {
    ...
    (_a = this.getRequiredNodes()) === null || _a === void 0 ? void 0 : _a.forEach((item) => {
      new preciseCssSetter.PreciseCssSetter(item)
        .set(REQUIREMENT_FULFILLED_CLASS, this.internalRequirementFulfilled)
        .execute();
    });
  }
  connectedCallback() {
    this.slotObserver = new slotObserver.SlotObserver(this.self, "ds-bordered-form-item", (nodes) => {
      this.nodes = nodes;
  ...
  computeBorders() {
    ...
    nodes.forEach((node, index) => {
      ...
      new preciseCssSetter.PreciseCssSetter(node)
        .set("first", markFirst)
        .set("last", markLast)
  ...
  render() {
    ...
    return (index.h(index.Host, { class: Object.assign(Object.assign({}, directionClass), { "parent-size": this.size === "parent", required: this.containsRequiredField }) }, index.h("div", { class: "container" }, index.h("div", { class: "wrapper", ref: (el) => (this.wrapperElement = el) }, index.h("slot", null), index.h(errorPopup.ErrorPopup, { text: this.errormessage, show: this.currentFocusedIndex !== -1 })), index.h("ds-required-hint", { required: this.containsRequiredField, fulfilled: this.internalRequirementFulfilled }))));
  }
...
const DsBorderedFormItem = class {
  ...
  configureStates() {
    ...
    new preciseCssSetter.PreciseCssSetter(this.self)
  ...
  render() {
    return (index.h(index.Host, { onClick: (e) => this.delegateEvent(e), onMouseDown: (e) => this.delegateEvent(e), onMouseEnter: (e) => this.delegateEvent(e), onMouseLeave: (e) => this.delegateEvent(e) }, 
    // TODO: Find better solution
    !window["chrome"] ? index.h(baseElements.BaselineElement, { onlyDiv: true }) : null, index.h("div", { ref: (el) => (this.contentElement = el), class: "content-container" }, index.h("slot", null)), index.h(errorPopup.ErrorPopup, { text: this.errormessage, show: this.inputFocused })));
  }
  get self() { return index.getElement(this); }
};
DsBorderedFormItem.style = dsBorderedFormItemCss;

exports.ds_bordered_form_group = DsBorderedFormGroup;
exports.ds_bordered_form_item = DsBorderedFormItem;
