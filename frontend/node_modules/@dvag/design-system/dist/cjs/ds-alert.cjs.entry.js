...
const DsAlert = class {
  constructor(hostRef) {
    ...
    this.submitResult = new multicastObservable.DsSubject();
    this.customShowCalled = false;
    this.transitionStartHelper = new event_util.TransitionStartHelper(this, this.handleTransitionStart);
  ...
  connectedCallback() {
    this.slotObserver =
      slotObserver.createMetaComponentsSlotObserver(this.self, "ds-alert-button", async (nodes) => {
  ...
  onKeyDown(e) {
    if (this.visible && e.key === "Enter") {
      ...
      if (explicitTargetElement !== this.self) {
        const isTargetElementWithinSelf = miscUtil.isInnerElement(explicitTargetElement, this.self);
   ...
   */
  async show() {
    assert.assert(this.customShowCalled != true, () => `<ds-alert> show() sollte nicht benutzt werden, wenn vorher einer der vordefinierten showAlert() || showConfirm() Methoden benutzt wurde.`);
    return new Promise((resolve) => {
  ...
  render() {
    return (index.h(index.Host, { class: {
      ...
      }, tabIndex: 0 }, index.h("div", { class: "overlay" }), index.h("div", { class: "modal-layout", ref: (ref) => this.transitionStartHelper.on(ref), onTransitionEnd: (e) => this.handleTransitionEnd(e) }, index.h("ds-card", { class: "modal-element", ref: (el) => (this.modalDialog = el) }, this.ignoreSlots && this.customRenderFunction !== undefined
      ? this.customRenderFunction()
      : this.renderDefault()))));
  }
  renderDefault() {
  ...
  renderIcon(icon, iconColor) {
    if (icon != null) {
      return (index.h("div", { class: "icon-container" }, index.h("ds-icon", { size: 32, icon: icon, color: iconColor })));
    }
  }
  renderIconSlot() {
    if (renderUtil.isSlotDefined(this.self, "icon")) {
      return (index.h("div", { class: "icon-container" }, index.h("div", null, index.h("div", null, index.h("div", null, index.h("slot", { name: "icon" }))))));
    }
  }
  renderDefaultContent() {
    const titleSlotDefined = renderUtil.isSlotDefined(this.self, "title");
    const bodySlotDefined = renderUtil.isSlotDefined(this.self, "body");
    return [
      titleSlotDefined || bodySlotDefined ? (index.h("div", { class: { content: true } }, titleSlotDefined ? (index.h("div", { class: "title" }, index.h("div", null, index.h("slot", { name: "title" })))) : null, bodySlotDefined ? (index.h("div", { class: "body" }, index.h("div", null, index.h("slot", { name: "body" })))) : null)) : null,
      index.h("slot", null),
    ];
  }
  renderContent(title, body) {
    return (index.h("div", { class: { content: true } }, index.h("div", { class: "title" }, index.h("ds-text", { type: "u5" }, title)), index.h("div", { class: "body" }, index.h("ds-text", { type: "ft" }, body))));
  }
  ...
  renderButton(buttonData, buttonIndex) {
    ...
    return (index.h("div", { class: {
      ...
      } }, index.h("ds-text-button", { id: elementIdHandling.ElementIdGenerator.createId(this.self, "button-" + buttonIndex), color: buttonData.color, disabled: buttonData.disabled }, index.h("ds-text", { type: buttonData.highlight ? "ftf" : undefined }, buttonData.label))));
  }
