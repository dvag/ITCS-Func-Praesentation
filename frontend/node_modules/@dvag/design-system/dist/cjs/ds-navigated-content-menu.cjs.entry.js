...
const DsNavigatedContentMenu = class {
  ...
  connectedCallback() {
    ...
    this.contentHeightBelowHeaderSubscription =
      headerService.subscribeToContentHeightBelowHeader((height) => (this.contentBelowHeaderHeight = height));
    const headerService$1 = headerService.HeaderService.instance();
    this.headerHeight = headerService$1.headerHeight;
    this.contentBelowHeaderHeight = headerService$1.contentHeightBelowHeader;
  ...
  registerMenuItemObserver() {
    this.menuItemObserver =
      slotObserver.createMetaComponentsSlotObserver(this.self, ":scope > ds-navigated-content-menu-item.hydrated", async (nodes) => {
        // The current implementation only support 2 levels of hierarchy
        this.menuItems = await Promise.all(nodes
          .map(async (node) => {
          return Object.assign(Object.assign({}, (await node.toMenuItemModel())), { children: await Promise.all(Array.from(node.querySelectorAll(queryUtil.notHidden("ds-navigated-content-menu-item"))).map((child) => child.toMenuItemModel())) });
        })
  ...
  measureChildrenBoxes() {
    ...
    if (diffChildHeights(childrenBoxHeights, this.childrenBoxHeights)) {
      renderUtil.onNextFrame(() => (this.childrenBoxHeights = childrenBoxHeights));
    }
  }
  pathForItem(item) {
  ...
  render() {
    return (index.h(index.Host, { style: {
      ...
      } }, index.h("ds-card", null, index.h("ds-card-content", null, this.menuItems.map((item) => this.renderMenuItem(item)), index.h("slot", { name: "bottom" })))));
  }
  renderMenuItem(item) {
    var _a;
    const path = this.pathForItem(item);
    ...
    return (index.h("div", { class: {
        ...
        "level-1": true,
      } }, index.h("div", { class: "menu-item-label" }, index.h("div", { class: "main" }, index.h("button", { type: "button", class: "navigate-button", id: elementIdHandling.ElementIdGenerator.createId(this.self, path), onClick: () => this.navigateToItem(item) }, index.h("ds-text", null, item.label), index.h("ds-icon", { icon: "drop-down", size: 16, class: {
  ...
  renderItemChildren(children, parentValue, isExpanded) {
    ...
    return [
      ...
      index.h("div", { class: {
        ...
        } }, index.h("div", { class: "slider" }, index.h("div", { class: "line" }), index.h("div", { class: "knob", ref: (el) => this.registerKnob(parentValue, el) })), index.h("div", { class: "items" }, children.map((item) => this.renderChild(item, isExpanded)))),
    ];
  }
  renderItemChildrenMeasurementBox(children, parentValue) {
    return (index.h("div", { class: "measurement-box", "data-measurementbox": parentValue, ref: (ref) => {
        this.resizeObserver.disconnect();
        if (ref != null) {
          this.resizeObserver.observe(ref);
        }
      } }, index.h("div", { class: "items" }, children.map((item) => this.renderChild(item, false)))));
  }
  renderChild(item, isInteractive) {
    ...
    return (index.h("div", { class: {
        "menu-item": true,
        "level-2": true,
      }, "data-path": path }, index.h("div", { class: "menu-item-label" }, index.h("div", { class: "main" }, index.h("button", { type: "button", class: "navigate-button", tabIndex: isInteractive ? 0 : -1, id: elementIdHandling.ElementIdGenerator.createId(this.self, path), onClick: () => {
        if (isInteractive) {
          this.navigateToItem(item);
        }
      } }, index.h("ds-text", null, item.label))), this.renderStatusIcons(item))));
  }
  renderStatusIcons(item) {
    const isError = item.error;
    const isWarning = !isError && item.warning;
    const isSuccess = !isError && !isWarning && item.success;
    return (index.h("div", { class: "status" }, isWarning ? (index.h("ds-icon", { icon: "achtung-circle", color: "gelb-1", size: 16 })) : null, isError ? (index.h("ds-icon", { icon: "achtung", color: "rot-1", size: 16 })) : null, isSuccess ? (index.h("ds-icon", { icon: "success", color: "gruen-1", size: 16 })) : null));
  }
...
exports.ds_navigated_content_menu = DsNavigatedContentMenu;
