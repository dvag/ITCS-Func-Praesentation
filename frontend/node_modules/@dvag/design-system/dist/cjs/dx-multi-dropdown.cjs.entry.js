...
const DxMultiDropdown = class {
  ...
  getInnerDropdownOptionElements() {
    return this.self.querySelectorAll(queryUtil.notHidden("dx-dropdown-option"));
  }
  isChevronRotated() {
  ...
  render() {
    var _a, _b, _c, _d;
    if (this.readonly) {
      return (index.h(baseElements.ReadonlyTextElement, { text: (_a = this.selectedDropdownOptions
    ...
    return (index.h(index.Host, { class: {
        ...
        "size-s": this.size === "s",
        "size-m": this.size === "m",
      }, ref: (el) => (this.inputElement = el), id: elementIdHandling.ElementIdGenerator.createId(this.self, "user-interaction"), tabIndex: this.disabled ? -1 : 0, onKeyDown: (event) => this.onInputKeyDown(event), onKeyUp: (event) => this.onInputKeyUp(event), onFocus: () => this.onInputFocused(), onBlur: () => this.onInputBlurred(), onClick: () => this.onInputClicked() }, index.h("dx-list-flyout", { open: this.listOpen, position: this.popupposition, direction: this.textalign === "left" ? "right" : "left", itemsvisible: 5.5, size: "m", selectionmode: "multi", stopclickpropagationonpopup: true, class: {
      ...
      } }, this.renderNativeSelect(), this.renderCurrentValue(), this.renderOpenListButton()), this.renderItemList())), index.h(error.ErrorMessage, { self: this.self, message: this.errormessage, custom: {
        onFocus: () => {
  ...
  renderItemList() {
    return this.options.map((option, index$1) => {
      return (index.h("dx-list-item", { id: elementIdHandling.ElementIdGenerator.createId(this.self, `list-item-${index$1}`), type: "checkbox", value: option.value, label: option.label, icon: option.icon, textalign: this.textalign, selected: option.selected }));
    });
  }
  renderOption(option, isPlaceholder = false) {
    return (index.h("div", { id: !isPlaceholder
      ...
      } }, option.icon != null ? index.h("dx-icon", { size: 24, icon: option.icon }) : null, index.h("dx-text", { key: option.label, class: { label: true, "align-right": this.textalign === "right" } }, option.label)));
  }
  renderCurrentValue() {
...
DxMultiDropdown.style = dxMultiDropdownCss;

exports.dx_multi_dropdown = DxMultiDropdown;
