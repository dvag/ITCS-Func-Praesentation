...
const DxTabBar = class {
  ...
  connectedCallback() {
    ...
    this.updateViewMode();
    this.parentIsCard = miscUtil.tagOf(this.self.parentElement) === "dx-card";
  }
  ...
  componentShouldUpdate(_, __, propName) {
    if (propName === "viewMode") {
      const innerActiveElement = miscUtil.getInnerActiveElement(this.self);
  ...
  initializeObservers() {
    ...
    this.slotObserver = new slotObserver.SlotObserver(this.self, ":scope > dx-tab-page", async (nodes) => {
      var _a;
  ...
  renderBarItem(tabPage) {
    ...
    return (index.h("div", { class: { "bar-item": true, selected: isSelected } }, index.h("button", { id: elementIdHandling.ElementIdGenerator.createId(this.self, `tab-${tabPage.value}`), key: tabPage.value, class: {
        "bar-item-button": true,
        selected: isSelected,
      }, disabled: isSelected, onClick: (e) => {
        this.setActiveTab(tabPage);
        scrollElementIntoView(e.target);
      } }, index.h("dx-text", { type: "ps", class: "label" }, tabPage.label)), index.h("div", { class: { "selected-state-line-container": true } }, index.h("div", { class: { "selected-state-line": true, selected: isSelected } }))));
  }
  renderScrollButton(position) {
    return (index.h("button", { id: elementIdHandling.ElementIdGenerator.createId(this.self, `scroll-button-${position}`), class: {
        "scroll-button": true,
        left: position === "left",
        right: position === "right",
        show: (this.showLeftButton && position === "left") ||
          (this.showRightButton && position === "right"),
      }, onClick: () => this.onScrollButtonClicked(position) }, index.h("dx-icon", { icon: `chevron-${position === "left" ? "links" : "rechts"}`, size: 24 })));
  }
  renderTabBarItems() {
    return (index.h("div", { class: "clip-container", onScroll: (e) => {
      ...
      }, onScroll: (e) => this.onScrollTabBar(e) }, index.h("div", { class: "page-container", ref: (ref) => {
        if (ref != null) {
          this.resizeObserver.observe(ref);
        }
      } }, index.h("div", { class: "tab-bar-items-container" }, this.tabPages.map((tabPage, index$1) => {
      return [
        index$1 !== 0 ? index.h("div", { class: { spacer: true } }) : null,
        this.renderBarItem(tabPage),
      ];
    }))))));
  }
  renderTabs() {
    return (index.h(index.Fragment, null, this.renderScrollButton("left"), this.renderTabBarItems(), this.renderScrollButton("right")));
  }
  renderDropdown() {
    return (index.h("dx-tab-dropdown", { class: "tab-dropdown", label: this.getLabel(this.selected), key: tabPageArrayHash(this.tabPages), onValueComplete: (e) => {
        this.selected = e.detail.value;
      } }, this.tabPages.map((tabPage) => (index.h("dx-dropdown-option", { label: tabPage.label, value: tabPage.value })))));
  }
  renderTabBar() {
  ...
  render() {
    return (index.h(index.Host, { class: {
      ...
      } }, this.renderTabBar(), index.h("div", { class: "tab-page-content" }, index.h("div", { class: "page-container" }, index.h("slot", null)))));
  }
