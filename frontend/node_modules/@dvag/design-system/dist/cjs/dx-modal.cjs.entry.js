...
const DxModal = class {
  ...
  controlOpticalHints() {
    ...
    requestAnimationFrame(() => {
      new preciseCssSetter.PreciseCssSetter(this.topShadowElement)
        .set("visible", showHeaderShadow)
        .execute();
      new preciseCssSetter.PreciseCssSetter(this.bottomShadowElement)
  ...
  render() {
    assert.assertPropertyValueByPredicate("dx-modal", "height", this.height, () => {
      return (["fullscreen", "content", "internal-site-navigation-menu"].includes(this.height) || !isNaN(parseInt(this.height)));
    });
    const isRightContainerDefined = renderUtil.isSlotDefined(this.self, PRIMARY_ACTIONS_SLOT);
    const isLeftContainerDefined = renderUtil.isSlotDefined(this.self, SECONDARY_ACTIONS_SLOT);
    ...
    return (index.h(index.Host, { style: {
      ...
      } }, index.h("div", { class: "overlay" }), index.h("div", { class: "modal-layout", onClick: (e) => this.closeModalOnClickOutside(e), onAnimationEnd: () => this.onLayoutAnimationEnd() }, index.h("div", { ref: (el) => (this.modalDialog = el), class: {
        "modal-element": true,
  ...
  renderHeader() {
    return (index.h("div", { class: "header" }, index.h("div", { class: "header-content" }, index.h("div", null, index.h("dx-text", { type: "ps" }, this.label)), index.h("div", { class: "header-side" }, index.h("dx-icon", { onClick: () => this.closeModal("button"), id: elementIdHandling.ElementIdGenerator.createId(this.self, "close-button"), icon: "schliessen", size: 24, color: "default" }))), index.h("div", { class: "header-border" })));
  }
  renderContent(countOfDefinedSlots) {
    return (index.h("div", { class: "modal-content" }, index.h("div", { class: { topShadow: true }, ref: (ref) => (this.topShadowElement = ref) }), index.h("ds-scroll-container", { scrollbaroffset: 4, scrollbarwidth: 8, thumbwidth: 8, scrollbarmargin: 8, scrollbarborderradius: 0 }, index.h("div", { ref: (el) => {
        this.scrollableElement = el;
        this.setScrollContainer(el);
      }, class: {
        content: true,
        "no-footer": countOfDefinedSlots === 0,
      } }, index.h("slot", { name: "content" }))), index.h("div", { class: { bottomShadow: true }, ref: (ref) => (this.bottomShadowElement = ref) })));
  }
  renderFooter(countOfDefinedSlots) {
    return (index.h("div", { class: {
      ...
      } }, index.h("div", { class: "footer-border" }), index.h("div", { class: "footer-content" }, index.h("div", { class: "footer-left-content" }, index.h("slot", { name: SECONDARY_ACTIONS_SLOT })), index.h("div", { class: "footer-right-content" }, index.h("slot", { name: PRIMARY_ACTIONS_SLOT })))));
  }
