...
const DsNumberInput = class {
  ...
  getDisplayValue(value = null) {
    return currencyUtil.displayNumberAsCurrency(value !== null && value !== void 0 ? value : this.value, this.inputTypeDefinition.decimalDigits, this.inputTypeDefinition.thousandsSeparator);
  }
  ...
  render() {
    if (this.readonly) {
      return (index.h("div", null, index.h(baseElements.ReadonlyTextElement, { text: this.formattedValue, label: this.label, kisynced: this.kisynced }), index.h(baseElements.ReadonlyIconElement, { icon: this.inputTypeDefinition.icon })));
    }
  ...
  renderInput() {
    ...
    return (index.h("label", null, index.h("ds-input-label", { disabled: this.disabled, label: this.label, kisynced: this.kisynced }), index.h(inputContainer.InputContainer, { active: this.inputFocused, error: isErrorOutline, required: this.required, requirementFulfilled: requirementFulfilled, disabled: this.disabled, title: (_b = this.value) === null || _b === void 0 ? void 0 : _b.toString() }, index.h("div", { class: "input-container" }, index.h("input", { id: elementIdHandling.ElementIdGenerator.createId(this.self, "control"), ref: (el) => (this.inputElement = el), onKeyUp: (event) => this.onInputKeyUp(event), onInput: () => this.onInput(), onFocus: () => this.onInputFocused(), onBlur: () => this.onInputBlurred(), type: "text", inputMode: "numeric", pattern: "[0-9]*", value: this.inputFocused
      ...
      }, lang: "de-DE", disabled: this.disabled }), this.renderIcon()), index.h(errorPopup.ErrorPopup, { text: this.errormessage, show: this.inputFocused, rightAligned: true })), index.h("ds-required-hint", { required: this.required, fulfilled: requirementFulfilled })));
  }
  ...
  renderIconSpecified() {
    ...
    return (index.h("div", { class: "right-icon" }, this.isClearButtonVisible() ? (index.h("ds-function-icon-button", { id: elementIdHandling.ElementIdGenerator.createId(this.self, "clear-button"), unfocusable: true, class: {
        "clear-button": true,
        visible: true,
      }, onPointerDown: () => this.clearInput() }, index.h("ds-icon", { icon: "schliessen-input", size: 16 }))) : icon != "" ? (index.h("ds-icon", { class: { "type-icon": true }, icon: icon, color: "gold", size: 16 })) : null));
  }
  renderIcon() {
...
exports.ds_number_input = DsNumberInput;
