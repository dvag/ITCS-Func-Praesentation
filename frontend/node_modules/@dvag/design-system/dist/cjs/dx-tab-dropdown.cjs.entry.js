...
const DxTabDropdown = class {
  ...
  renderItemList() {
    var _a;
    return (_a = this.options) === null || _a === void 0 ? void 0 : _a.map((option, index$1) => {
      return (index.h("dx-list-item", { id: elementIdHandling.ElementIdGenerator.createId(this.self, `list-item-${index$1}`), type: "selectable", value: option.value, label: option.label, icon: option.icon, onClick: () => this.closeListAndEmitSelectedOption(option), selected: this.selectedOption === option }));
    });
  }
  renderBaseSlot() {
    return (index.h("button", { onClick: () => (this.internalOpen = !this.internalOpen), slot: "base", class: { "base-component": true } }, this.renderCurrentValue(), this.renderOpenListButton()));
  }
  renderOpenListButton() {
    return (index.h("div", { class: {
        "open-list-button": true,
        "list-open": this.internalOpen,
      } }, index.h("dx-chevron", { class: "chevron", directiontop: this.internalOpen })));
  }
  renderCurrentValue() {
    return (index.h("div", { class: { "current-value": true } }, index.h("dx-text", null, this.label), index.h("div", { class: "golden-line-container" }, index.h("div", { class: "golden-line" }))));
  }
  render() {
    return (index.h(index.Host, { onKeyDown: (e) => this.onInputKeyDown(e), onKeyUp: (e) => this.onInputKeyUp(e), tabIndex: 0 }, index.h("dx-list-flyout", { class: "list-flyout", itemsvisible: 5.5, size: "m", selectionmode: "single", open: this.internalOpen }, this.renderBaseSlot(), this.renderItemList())));
  }
  get self() { return index.getElement(this); }
};
DxTabDropdown.style = dxTabDropdownCss;

exports.dx_tab_dropdown = DxTabDropdown;
