...
import { angleToDirection, directionToRadians, normalize, PI, Point, toPoint, } from "../../../classes/math-util";
...
export class DxDonutChart {
  constructor() {
    ...
    this.debounce = new Debounce(200);
  ...
  onShowSegmentLabelsChanged() {
    if (this.showsegmentlabels) {
      ...
      onNextFrame(() => this.startUpdateCalculation());
    }
  }
  connectedCallback() {
    this.breakpointAware = new BreakpointAware(() => null, true);
  }
    ...
    return new Promise((resolve) => {
      readTask(() => {
        ...
        const writeQueue = normalizeOffset(updatePositionsInfo).map((data, index) => {
          ...
          const origin = new Point(this.viewBoxSize * 0.5);
          ...
          const directionFromLineStartToEnd = normalize(lineDirection);
          ...
          return () => {
            if (correspondingLabelLine != null) {
              new PreciseCssSetter(correspondingLabelLine)
            ...
            const angleInRadians = directionToRadians(directionFromLineStartToEnd);
  ...
  componentDidLoad() {
    this.slotObserver =
      createMetaComponentsSlotObserver(this.self, notHidden("dx-chart-sample"), async (nodes) => {
  ...
  async collectSamplesFromChildren(nodes) {
    ...
    assert(this.chartInput.length >= 2 && this.chartInput.length <= 8, () => "A donut chart should contain 2 to 8 samples!", this.self);
  }
  getHitAngle(mouseX, mouseY) {
    const selfBoundingClientRect = this.self.getBoundingClientRect();
    const relativeOrigin = new Point(selfBoundingClientRect.width * 0.5, selfBoundingClientRect.height * 0.5);
    const clientOrigin = relativeOrigin.plus(toPoint(selfBoundingClientRect));
    ...
    if (lengthSquared >= minRadiusSquared &&
      ...
      let angle = (Math.acos(direction.x / Math.sqrt(lengthSquared)) * 180) / PI;
  ...
  getColor(segment, selected) {
    const getColorPaletteName = () => {
      ...
      switch (this.colorscheme) {
        ...
        default:
          throwCompileErrorIfReachable(this.colorscheme);
      }
    };
    return new ColorPalette(true).getColorCode(getColorPaletteName());
  }
  ...
  renderLineFromDirectionVector(origin, directionVector) {
    var _a;
    return (h("line", { x1: origin.x, y1: origin.y, x2: origin.x + directionVector.x, y2: origin.y + directionVector.y, "stroke-width": (_a = window.offsetValue) !== null && _a !== void 0 ? _a : 1, stroke: "black" }));
  }
  renderCircleSegment(data, sumOfAllSegments, selected) {
    ...
    return (h("circle", { class: "segment", cx: this.viewBoxSize * 0.5, cy: this.viewBoxSize * 0.5, r: "15.915", fill: "transparent", "stroke-dasharray": dashArray, "stroke-dashoffset": dashOffset, "stroke-width": selected
  ...
  renderSegmentMask(createData) {
    ...
    return (h("mask", { id: "segment-offset" }, h("rect", { width: "100%", height: "100%", fill: "white" }), segments.map((segment) => {
      const directionToEnd = this.getSegmentEndDirectionFromOrigin(segment, createData.sum);
      const donutDiameter = this.donutRadius * 2;
      return this.renderLineFromDirectionVector(new Point(this.viewBoxSize * 0.5), new Point(directionToEnd.x * donutDiameter, directionToEnd.y * -donutDiameter));
    })));
  }
  renderLabelLine() {
    if (!this.showsegmentlabels) {
    ...
    return h("line", { class: "label-line" });
  }
  renderCircleSegments(createData) {
    return [
      h("svg", { viewBox: `0 0 ${this.viewBoxSize} ${this.viewBoxSize}` }, h("defs", null, this.renderSegmentMask(createData)), createData.segments.map((segment, i) => this.renderCircleSegment(segment, createData.sum, (this.internalSelected == null && i === this.selected) ||
        i === this.internalSelected))),
      h("svg", { class: "label-svg" }, createData.segments.map(() => this.renderLabelLine())),
    ];
  }
  renderSegmentLabels(createData) {
    ...
    return createData.segments.map((segment) => {
      return (h("div", { class: "segment-label can-be-visible" }, segment.input.icon && (h("dx-icon", { color: "paragraph", icon: segment.input.icon })), h("dx-text", { color: "paragraph" }, segment.input.label)));
    });
  }
  renderLabels(createData) {
    var _a, _b;
    return (h("div", { class: "overlay" }, h("div", { class: "inner-labels" }, ((_a = this.label) === null || _a === void 0 ? void 0 : _a.length) > 0 ? (h("dx-text", { class: "label" }, this.label)) : null, ((_b = this.sublabel) === null || _b === void 0 ? void 0 : _b.length) > 0 ? (h("dx-text", { class: "sub-label" }, this.sublabel)) : null), this.renderSegmentLabels(createData)));
  }
  render() {
    const createData = this.generateDonutCreateData();
    return (h(Host, { style: {
        "user-select": "none",
      }, onContextMenu: () => false, onPointerEnter: (e) => this.handlePointerEnter(e), onPointerLeave: (e) => this.handlePointerLeave(e), onPointerUp: (e) => this.handlePointerUp(e) }, h("div", { class: "donut-container" }, this.renderCircleSegments(createData), this.renderLabels(createData))));
  }
  static get is() { return "dx-donut-chart"; }
    ...
    return {
      "selected": {
        ...
        "docs": {
          "tags": [{
              "name": "value",
              "text": "number: An index between 0 and (n - 1), whereas n = number of segments."
            }, {
              "name": "example",
              "text": "selected"
            }],
          "text": "The index of the selected segment."
        },
        "attribute": "selected",
        "reflect": true,
        "defaultValue": "null"
      ...
      "colorscheme": {
        ...
        "mutable": false,
        "complexType": {
          "original": "DonutChartColorScheme",
          "resolved": "\"blue\" | \"custom\" | \"gray\" | \"signal\"",
          "references": {
        ...
        "docs": {
          "tags": [{
              "name": "value",
              "text": "blue: All segments are displayed in a predefined blue color"
            }, {
              "name": "value",
              "text": "gray: All segments are displayed in a predefined gray color"
            }, {
              "name": "value",
              "text": "signal: The segments can be displayed in the three different signal colors (success, warning, error)."
            }, {
              "name": "value",
              "text": "custom: The color of segments can be set to any color in the color palette."
            }, {
              "name": "example",
              "text": "color-scheme"
            }, {
          ...
          "text": "The color scheme which is used to color the segments"
        },
      ...
      "label": {
        ...
        "docs": {
          ...
          "text": "A label which is displayed in the center of the component."
        },
        "attribute": "label",
        "reflect": false
      },
      "sublabel": {
        ...
        "docs": {
          "tags": [{
              "name": "value",
              "text": "string: The sub-label text."
            }, {
              "name": "example",
              "text": "label"
            }],
          "text": "A sub-label which is displayed below the `label` in the center of the component."
        },
        "attribute": "sublabel",
        "reflect": false
      },
      "showsegmentlabels": {
        ...
        "docs": {
          "tags": [{
              "name": "value",
              "text": "true: The label of every segment is displayed."
            }, {
              "name": "value",
              "text": "false: No segment labels are visible."
            }, {
              "name": "example",
              "text": "show-segment-labels"
            }],
          "text": "Defines whether to show the outer labels of the segments or not."
        },
        "attribute": "showsegmentlabels",
        "reflect": false,
        "defaultValue": "false"
      }
    };
  ...
  static get watchers() {
    return [{
        "propName": "showsegmentlabels",
        "methodName": "onShowSegmentLabelsChanged"
      }];
  }
  static get listeners() {
    return [{
        "name": "resize",
        "method": "onWindowResizeChange",
        "target": "window",
        "capture": false,
        "passive": true
      }];
