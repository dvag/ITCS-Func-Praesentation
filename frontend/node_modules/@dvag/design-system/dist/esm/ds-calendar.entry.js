...
import { D as DateUtils, M as MONTH_NAMES, a as MONTH_NAMES_SHORT } from './date.utils-73541332.js';
import { E as ElementIdGenerator } from './element-id-handling-bb37154e.js';
import { f as findUpperElementByPredicate } from './misc-util-d3b69f2c.js';
import { e as extractDateRange } from './date-range.util-9056a753.js';
...
const DsCalendar = class {
  ...
  connectedCallback() {
    this.parent = findUpperElementByPredicate(this.self, (elem) => {
      ...
      return tag === "ds-date-input" || tag === "ds-date-range";
    });
  }
  componentWillLoad() {
  ...
  renderRowEntry(rowEntry) {
    ...
    if (isDateEntry(rowEntry)) {
      return (h("div", { class: {
        ...
        } }, rowEntry.interactable ? (h("ds-text-button", { key: `${DateUtils.dateToString(rowEntry.date)}-${this.currentView}`, id: ElementIdGenerator.createId(this.self, `row-entry-${DateUtils.dateToString(rowEntry.date)}`), unfocusable: true, screensize: "small", class: {
          "calendar-entry": true,
          "bright-color": rowEntry.bright,
          active: rowEntry.active,
        }, onClick: () => this.handleRowEntryClick(rowEntry.date) }, h("ds-text", { type: (_a = rowEntry.textType) !== null && _a !== void 0 ? _a : "it" }, rowEntry.label))) : (h("ds-text", { type: (_b = rowEntry.textType) !== null && _b !== void 0 ? _b : "it" }, rowEntry.label))));
    }
    if (isBoundingEntry(rowEntry)) {
      return (h("div", { class: {
          "calendar-entry-container": true,
        } }, h("ds-icon", { size: 24, icon: "listenpfeil", class: {
          "calendar-entry": true,
          bound: true,
          max: rowEntry === "max",
        } })));
  ...
  renderErrorMessage() {
    ...
    if (((_a = this.errormessage) === null || _a === void 0 ? void 0 : _a.length) > 0) {
      return (h("ds-text", { class: "error-message", type: "itk" }, this.errormessage));
    }
  }
  renderCalendar(calendar) {
    return (h("ds-grow-box", { animateoninit: false, slidecontent: false }, h("div", { class: {
        calendar: true,
        "day-view": this.internalView === "day",
        "month-view": this.internalView === "month",
        "year-view": this.internalView === "year",
  ...
  renderHeader(label, leftButtonEnabled, rightButtonEnabled) {
    return (h("div", { class: {
        header: true,
      } }, h("ds-function-icon-button", { unfocusable: true, onClick: () => this.navigate("left"), class: "left", disabled: !leftButtonEnabled, id: ElementIdGenerator.createId(this.self, `left-button`) }, h("ds-icon", { icon: "listenpfeil", size: 24 })), h("div", { class: "label-container" }, h("ds-text-button", { class: "label", ref: (ref) => (this.headerLabel = ref), id: ElementIdGenerator.createId(this.self, `header-label-button`), onClick: () => this.onClickHeaderLabel(), unfocusable: true, color: "gold" }, h("ds-text", null, label))), h("ds-function-icon-button", { unfocusable: true, onClick: () => this.navigate("right"), disabled: !rightButtonEnabled, id: ElementIdGenerator.createId(this.self, `right-button`) }, h("ds-icon", { icon: "listenpfeil", size: 24 }))));
  }
  render() {
...
DsCalendar.style = dsCalendarCss;

export { DsCalendar as ds_calendar };
