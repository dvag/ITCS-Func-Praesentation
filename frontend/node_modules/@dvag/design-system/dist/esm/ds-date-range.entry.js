...
const DsDateRange = class {
  ...
  connectedCallback() {
    this.slotObserver = new SlotObserver(this.self, "ds-date-input", (dateInputs) => {
      assert(dateInputs.length === 2, () => "You have to define exactly two ds-date-inputs within this component");
  ...
  handleBlur() {
    onNextFrame(() => {
      const loseFocus = document.activeElement !== this.self &&
        !findUpperElementByPredicate(document.activeElement, (elem) => elem === this.self);
      if (loseFocus) {
        this.currentlyFocused = false;
      }
    });
  ...
  renderCalendar() {
    ...
    const dateInputs = this.getDateInputs();
    return (h("ds-calendar", { slot: "overlay", id: ElementIdGenerator.createId(this.self, "calendar"), ref: (ref) => this.setCalendar(ref), tabindex: -1, errormessage: this.internalErrorMessage ||
  ...
  render() {
    return (h(Host, null, h("dx-overlay", { position: "bottom", offsety: 8, disabled: !this.currentlyFocused }, h("div", { class: "date-container" }, h("div", { class: "date-input-container" }, h("slot", { name: "from" })), h("div", { class: "date-input-container" }, h("slot", { name: "to" }))), this.renderCalendar())));
  }
...
export { DsDateRange as ds_date_range };
