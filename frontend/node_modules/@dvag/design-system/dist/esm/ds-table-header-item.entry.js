...
const DsTableHeaderItem = class {
  ...
  componentWillLoad() {
    this.internAlign = this.getAlignment();
    assert(!!(this.label || this.columnid) || !this.sortable, () => "Sortierbare Spalten sollten die Eigenschaft 'columnid' oder 'label', zur Unterscheidung, definieren.");
    if (this.sortable) {
      verifyElementId(this.self);
    }
  }
  ...
  render() {
    ...
    return (h(Host, { style: this.buildStyle(), class: alignClass, role: "columnheader" }, h("button", { id: ElementIdGenerator.createId(this.self, "sort-button"), type: "button", disabled: !this.sortable, onClick: () => this.invokeSort(), class: "header" }, h("div", { class: Object.assign({ cell: true }, alignClass) }, h("div", { class: "label-container" }, h("div", { class: {
        label: true,
  ...
  renderLabel() {
    if (this.label != null) {
      return (h("ds-text", { type: "it", color: this.sortState !== "neutral" ? "gold" : undefined }, this.label));
    }
  }
  renderSortButton() {
    if (this.sortable) {
      return h("ds-icon", { class: "sort-icon", icon: this.getSortIcon(), size: 16 });
    }
  }
  getNextSortState() {
    if (this.sortState == "neutral") {
...
export { DsTableHeaderItem as ds_table_header_item };
